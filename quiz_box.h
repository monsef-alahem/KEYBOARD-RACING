...
int quiz_box_clean_lines()
{
    ...
    for (i=0;i<CHAT_LINES;i++)
        memset(quiz_box.text[i],0,CHAT_LINE_LEN);
}
...
void init_quiz_box()
{
    ...
    strcpy(quiz_box.text[0], level_text[0]);
}
...
void update_quiz_box()
{
    ...
    for (i=0;i<CHAT_LINES;i++) {
        SDL_Color text_color = {0,255,100};
        quiz_box.line[i].sur = TTF_RenderText_Solid(quiz_box.font, quiz_box.text[i], text_color);
